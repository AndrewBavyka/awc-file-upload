    ...
    protected render(): TemplateResult {
        ...
        return html`
            <div class="awc-file-upload-selected">
                ${selectedFiles.map(({ file, provider, providerIcon }) => html`
                    <div class="awc-file-upload-selected__file">
                        <div class="awc-file-upload-selected__preview">
                            <img src="${file.thumbnail || 'fallback-icon'}" alt="${file.name}" />
                            <span class="awc-file-upload-selected__provider">${providerIcon}</span>
                        </div>
                        <div class="awc-file-upload-selected__info">
                            <span class="awc-file-upload-selected__name">${file.name}</span>
                            <span class="awc-file-upload-selected__size">${this.formatFileSize(file.size || 0)}</span>
                        </div>
                        <div class="awc-file-upload-selected__delete">
                            ${clearIcon}
                        </div>
                    </div>
                `)}
